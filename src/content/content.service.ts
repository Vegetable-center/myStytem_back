...
export class ContentService {
  ...
  async findAll() {
    ...
    const posts = await this.postRepository
      ...
      .andWhere('content.status = :status', { status: PostStatus.NORMAL })
  ...
  async getContentById(id: number){
    ...
    if (post.status === PostStatus.BANNED) {
      throw new NotFoundException('帖子不存在或已被封禁');
    }
    post.viewCount += 1;
  ...
  // 根据id获取帖子信息
  async getContentByIdAdmin(id: number){
    // 构建查询：关联用户表 + 筛选字段（排除userId）
    const post = await this.postRepository
      .createQueryBuilder('content')
      .leftJoin('content.user', 'user') // 关联用户表
  ...
  // ✅ 新增：封禁帖子
  async banPost(postId: number) {
    const post = await this.postRepository.findOne({ where: { id: postId } });
    if (!post) {
      throw new NotFoundException('帖子不存在');
    }
    if (post.status === PostStatus.BANNED) {
      throw new ForbiddenException('帖子已经是封禁状态');
    }
  ...
  // ✅ 新增：解封帖子
  async unbanPost(postId: number) {
    const post = await this.postRepository.findOne({ where: { id: postId } });
    if (!post) {
      throw new NotFoundException('帖子不存在');
    }
    if (post.status === PostStatus.NORMAL) {
      throw new ForbiddenException('帖子已经是正常状态');
    }
  ...
  async getBannedCount() {
    const count = await this.postRepository.count({
      where: { status: PostStatus.BANNED },
    });
    return {
      success: true,
      data: { count },
